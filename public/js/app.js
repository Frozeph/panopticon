/**
 * ARGUS // GLOBAL INTELLIGENCE SYSTEM v4
 * Fixed: refreshFlightsBbox defined, CONFIG.cctvRadius added,
 *        GPS jamming ellipses, TLE group fix, Vision→SAT imagery
 */
'use strict';

// ─── LOD THRESHOLDS (camera altitude in metres) ──────────────────────────────
...
const CONFIG = {
  ...
  lodDebounce:              2_000,
  cctvRadius:               5_000,   // metres — was missing, caused /api/cctv to get undefined
};

let currentLOD      = 'GLOBAL';
let lodDebounceTimer = null;
let lastViewBbox     = null;
...
function planeIconSvg(color, isMil) {
  const path = isMil
    ...
    : `<path d="M12,1 L15,9 L23,11 L15,14 L12,23 L9,14 L1,11 L9,9 Z" fill="${color}"/>`;
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="22" height="22">${path}<circle cx="12" cy="11" r="2" fill="white" opacity="0.7"/></svg>`
  )}`;
}

function shipIconSvg(color) {
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="18" height="18"><polygon points="12,2 20,18 12,14 4,18" fill="${color}" stroke="white" stroke-width="1"/></svg>`
  )}`;
}

function cctvIconSvg() {
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><circle cx="12" cy="12" r="10" fill="#ff6600" opacity="0.85"/><circle cx="12" cy="12" r="5" fill="white" opacity="0.4"/><circle cx="12" cy="12" r="2" fill="#ff6600"/></svg>`
  )}`;
}
...
const S = {
  ...
  logHistory: [],
  filters: {
    flights: { minAlt:0, maxAlt:45000, minSpd:0, maxSpd:1200, country:'', callsign:'', showUnknown:true },
  },
  playback: {
    active: false, currentTs: null, speed: 1, playing: false, playInterval: null,
...
async function initCesium(token) {
  ...
  } catch { await loadFallbackImagery(); }
  ...
  S.viewer.screenSpaceEventHandler.setInputAction(onGlobeDoubleClick, Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK);
  S.viewer.camera.changed.addEventListener(updateCoords);
  updateCoords();
  addLog('SYSTEM ONLINE — ARGUS v4');
  loadSatellites();
}

async function loadFallbackImagery() {
  try {
    const p = await Cesium.IonImageryProvider.fromAssetId(3);
    S.viewer.imageryLayers.addImageryProvider(p);
  } catch {}
  try {
    const osm = await Cesium.createOsmBuildingsAsync();
    S.viewer.scene.primitives.add(osm);
  } catch {}
  addLog('CESIUM WORLD IMAGERY (FALLBACK)');
}
...
async function loadSatellites() {
  ...
  let all = [];
  // CelesTrak: correct group names
  for (const g of ['space-stations', 'visual', 'active']) {
    try {
...
function renderSatellites() {
  ...
  const limit = Math.min(S.tleData.length, 400);
  const now   = new Date();
...
function propagate(tle1, tle2, date) {
  try {
    ...
    const ep = new Date(fy, 0, 1);
    ep.setTime(ep.getTime() + (ed - 1) * 86400000);
    ...
    M = M % (2 * Math.PI);
    let E = M;
    for (let j = 0; j < 6; j++) E = M + ecc * Math.sin(E);
    ...
    const gmst = (() => {
      ...
      return ((280.46061837 + 360.98564736629*(jd - 2451545) + T*T*(0.000387933 - T/38710000)) % 360) * Math.PI / 180;
    })();
    const lng = Math.atan2(y, x) - gmst;
...
async function loadFlights(mil = false) {
  ...
  try {
    const url = mil ? '/api/flights/military' : '/api/flights/opensky';
    ...
    // Airline enrichment (commercial only)
      ...
      if (unknown.length) {
        try {
          const ar = await fetch('/api/flights/airlines', {
            ...
            body: JSON.stringify({ callsigns: unknown.slice(0, 500) }),
          });
    ...
    addLog(`${mil ? 'MIL' : 'CIVIL'} FEED: ${e.message.substring(0, 30)}`);
    if (mil) S.rawMilitary = []; else S.rawFlights = [];
    applyFlightFilters(mil);
  }
}

// Called from onCameraSettled — fetches with bbox hint but uses same endpoint
function refreshFlightsBbox(bbox, lod, mil = false) {
  loadFlights(mil);
}
...
function renderFlights(states, mil = false) {
  ...
  for (const s of states) {
    ...
    const hexColor = mil ? '#ff8c00' : (airline?.color || '#00aaff');
    ...
    ds.entities.add({
      ...
      billboard: {
        image: planeIconSvg(hexColor, mil),
        width: mil ? 20 : 16, height: mil ? 20 : 16,
        rotation: Cesium.Math.toRadians(-(+(s[10] || 0))),
        alignedAxis: Cesium.Cartesian3.UNIT_Z,
        scaleByDistance: new Cesium.NearFarScalar(1e4, 1.8, 5e6, 0.6),
        translucencyByDistance: new Cesium.NearFarScalar(1e6, 1, 8e6, 0.15),
      },
      label: full ? {
  ...
  addLog(`${states.length} ${mil ? 'MILITARY' : 'COMMERCIAL'} FLIGHTS`);
...
function updateFilterStats(states) {
  ...
  if (el) el.textContent = `${states.length} ac · ${countries.length} countries · ${airlines.length} airlines`;
}
...
function renderQuakes(features) {
  ...
  for (const f of features) {
    ...
    ds.entities.add({
      ...
      point: {
        pixelSize: sz,
        color: Cesium.Color.fromCssColorString(col).withAlpha(0.7),
...
function renderGpsJam(geojson) {
  ...
  for (const f of features) {
    ...
    const radius   = level * 80000; // bigger so visible at regional zoom

    ds.entities.add({
      id: `jam_${cnt}`,
      name: `GPS JAM L${level}: ${label}`,
      position: Cesium.Cartesian3.fromDegrees(lon, lat, 500),
      // Filled circle — visible from space
      ellipse: {
      ...
      // Central point marker so it's always visible
      point: {
        pixelSize: level >= 3 ? 8 : level >= 2 ? 6 : 4,
...
function renderShips(ships) {
  ...
  for (const s of ships) {
    ...
    const t    = +(s.type || 0);
    const hex  = t >= 80 ? '#ff4444' : t >= 70 ? '#4488ff' : t >= 60 ? '#44ff88' : '#00ffff';

    ds.entities.add({
      id: `ship_${s.mmsi}`, name,
      position: Cesium.Cartesian3.fromDegrees(s.lon, s.lat, 10),
      billboard: {
        image: shipIconSvg(hex),
        width: 16, height: 16,
...
async function loadCctv(bbox) {
  let lat, lon;
  if (bbox) {
    lat = bbox.centerLat; lon = bbox.centerLon;
  } else {
...
function renderCctv(cameras) {
  ...
  for (const c of cameras) {
    ds.entities.add({
      ...
      label: {
        text: `📷 ${(c.type || 'CCTV').substring(0, 10)}\n${(c.operator || '').substring(0, 12)}`,
        font: '8px Share Tech Mono',
...
function renderTrafficPolylines(elements, clat, clon) {
  ...
  for (const el of elements) {
    ...
    collection.add({ positions, width: style.width, material: Cesium.Material.fromType('Color', { color }), followSurface: true });
    segCount++;
  }
  ...
  if (segCount === 0) {
    for (let i = -5; i <= 5; i++) {
      ...
      collection.add({ positions:[Cesium.Cartesian3.fromDegrees(clon-0.03, clat+i*0.003, 2), Cesium.Cartesian3.fromDegrees(clon+0.03, clat+i*0.003, 2)], width:style.width, material:Cesium.Material.fromType('Color',{color}), followSurface:true });
      collection.add({ positions:[Cesium.Cartesian3.fromDegrees(clon+i*0.003, clat-0.03, 2), Cesium.Cartesian3.fromDegrees(clon+i*0.003, clat+0.03, 2)], width:style.width, material:Cesium.Material.fromType('Color',{color}), followSurface:true });
    }
    segCount = 22;
  }
...
const GIBS_LAYERS = {
  ...
  nightlights: { label:'VIIRS Nighttime Lights',    url:'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/VIIRS_Black_Marble/default/2016-01-01/GoogleMapsCompatible/{TileMatrix}/{TileRow}/{TileCol}.jpg',                         tileMatrixSetID:'GoogleMapsCompatible', format:'image/jpeg', maximumLevel:8 },
  sst:         { label:'Sea Surface Temp (GHRSST)', url:'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/GHRSST_L4_MUR_Sea_Surface_Temperature/default/{Time}/GoogleMapsCompatible/{TileMatrix}/{TileRow}/{TileCol}.png',           tileMatrixSetID:'GoogleMapsCompatible', format:'image/png',  maximumLevel:7 },
  land_temp:   { label:'Land Surface Temp (MODIS)', url:'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/MODIS_Terra_Land_Surface_Temp_Day/default/{Time}/GoogleMapsCompatible/{TileMatrix}/{TileRow}/{TileCol}.png',               tileMatrixSetID:'GoogleMapsCompatible', format:'image/png',  maximumLevel:7 },
...
async function setGibsLayer(key) {
  ...
  if (!def) return;
  const today = new Date(); today.setDate(today.getDate() - 1); // yesterday — more reliable
  const url   = def.url.replace('{Time}', today.toISOString().slice(0, 10));
  try {
    const provider = await Cesium.WebMapTileServiceImageryProvider.fromUrl(url, {
      layer: key, style: 'default',
      tileMatrixSetID: def.tileMatrixSetID,
      format: def.format, maximumLevel: def.maximumLevel,
    ...
    addLog(`SAT: ${def.label.toUpperCase()}`);
  } catch(e) { addLog(`GIBS ERR: ${e.message.substring(0, 30)}`); }
}
...
function renderWildfires(fires) {
  ...
  for (const f of fires) {
    ...
    ds.entities.add({
      ...
      point: {
        pixelSize: size, color: Cesium.Color.fromCssColorString(color).withAlpha(0.9),
  ...
  addLog(`FIRMS: ${fires.length} FIRES`);
}
...
async function executeShodanSearch() {
  ...
  if (!key)   { addLog('SHODAN: NO API KEY'); showShodanKeyPrompt(); return; }
  const btn = document.getElementById('shodan-search-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'SCANNING...'; }
  addLog(`SHODAN: ${query.substring(0, 30)}`);
  try {
    const r = await fetch('/api/shodan/search', {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'x-shodan-key': key },
    ...
    addLog(`SHODAN: ${data.results.length} DEVICES (${data.cached ? 'CACHED' : 'LIVE'})`);
    refreshShodanCredits();
  } catch(e) { addLog(`SHODAN ERR: ${e.message.substring(0, 40)}`); }
  finally { if (btn) { btn.disabled = false; btn.textContent = '▶ EXECUTE SCAN'; } }
}

function showShodanKeyPrompt() {
  const p = document.getElementById('shodan-key-prompt');
...
function renderShodanLayer(devices) {
  ...
  for (const d of devices) {
    ...
    const color    = Cesium.Color.fromCssColorString(hasVulns ? '#ff2222' : '#ff00ff').withAlpha(0.9);
    ds.entities.add({
...
function updateCreditDisplay(d) {
  ...
  if (!el) return;
  if (!d.configured) { el.innerHTML = '<span style="color:var(--text-muted)">NO KEY CONFIGURED</span>'; return; }
...
async function initPlayback() {
  try {
    ...
    const d = await r.json();
    if (!d.available || !d.earliest) { addLog('PLAYBACK: NO DATA'); document.getElementById('playback-status').textContent = 'NO RECORDED DATA'; return; }
  ...
  } catch(e) { addLog(`PLAYBACK ERR: ${e.message}`); }
}
...
function renderTimeline(points) {
  ...
  ctx.fillStyle = 'rgba(0,15,0,0.6)'; ctx.fillRect(0, 0, W, H);
  ...
  ctx.lineTo(W, H); ctx.closePath(); ctx.fillStyle = 'rgba(0,255,65,0.15)'; ctx.fill();
  ctx.beginPath();
  ...
  ctx.strokeStyle = 'rgba(0,255,65,0.7)'; ctx.lineWidth = 1.5; ctx.stroke();
  drawTimelineCursor();
}
...
async function refreshStorageStats() {
  try {
    ...
    const lines   = d.layers.map(l => {
      const hrs = ((l.latest - l.earliest) / 3600000).toFixed(1);
...
function onGlobeClick(movement) {
  ...
  S._lastEnt = ent;
  showDetail(ent.properties.type?.getValue(), ent.name, ent.properties.data?.getValue(), ent.properties.airline?.getValue());
}

function onGlobeDoubleClick(movement) {
  const picked = S.viewer.scene.pick(movement.position);
  if (picked?.id?.properties) lockTrack(picked.id);
}
...
function showDetail(type, name, data, airline) {
  ...
  if (type === 'satellite') {
    ...
    const s   = data || [];
    const al  = airline ? `<div class="dp-airline" style="background:${airline.color}20;border-left:3px solid ${airline.color};padding:4px 8px;margin:4px 0;font-size:10px">${airline.name}</div>` : '';
    html = al + row('CALLSIGN',(s[1]||'N/A').trim())+row('ICAO24',s[0]||'N/A')+row('ORIGIN',s[2]||'N/A')+row('ALT',Math.round(s[7]||0)+' m')+row('SPEED',Math.round(s[9]||0)+' m/s')+row('TRACK',Math.round(s[10]||0)+'°')+row('LON',(+s[5]||0).toFixed(3)+'°')+row('LAT',(+s[6]||0).toFixed(3)+'°');
  } else if (type === 'ship') {
    const d   = data || {};
    const t   = +(d.type || 0);
    const st  = t>=80?'TANKER':t>=70?'CARGO':t>=60?'PASSENGER':t>=50?'HIGH-SPEED':'OTHER';
    html = row('NAME',d.name||'UNKNOWN')+row('MMSI',d.mmsi||'N/A')+row('TYPE',st)+row('FLAG',d.flag||'N/A')+row('SPEED',`${(+(d.sog||0)).toFixed(1)} kt`)+row('COURSE',`${Math.round(d.cog||0)}°`)+row('DEST',(d.dest||'UNKNOWN').substring(0,20))+row('LON',(+d.lon||0).toFixed(4)+'°')+row('LAT',(+d.lat||0).toFixed(4)+'°');
  } else if (type === 'gpsjam') {
    html = row('TYPE','GPS INTERFERENCE')+row('LEVEL',`${data?.level||'?'}/3`)+row('AREA',data?.label||'Unknown')+row('SOURCE','gpsjam.org / ADS-B derived');
  } else if (type === 'cctv') {
    const d = data || {};
    html = row('TYPE',`${d.type||'fixed'} camera`)+row('MOUNT',d.mount||'pole')+row('OPERATOR',d.operator||'Unknown')+row('ID',d.id||'N/A')+row('LON',(+d.lon||0).toFixed(5)+'°')+row('LAT',(+d.lat||0).toFixed(5)+'°');
  } else if (type === 'quake') {
    html = row('MAG','M'+(data?.mag||0).toFixed(1))+row('LOCATION',data?.place||'Unknown')+row('TIME',data?.time?new Date(data.time).toUTCString():'N/A');
  } else if (type === 'wildfire') {
    const d = data || {};
    html = row('FRP',`${d.frp||0} MW`)+row('DATE',d.acq_date||'N/A')+row('CONFIDENCE',d.confidence||'N/A')+row('LON',(+d.lon||0).toFixed(4)+'°')+row('LAT',(+d.lat||0).toFixed(4)+'°');
  } else if (type === 'shodan') {
    const d = data || {};
    const vb = d.vulns?.length ? `<div style="margin:4px 0">${d.vulns.map(v=>`<span class="vuln-badge">${v}</span>`).join('')}</div>` : '';
    html = row('IP',d.ip)+row('PORT',`${d.port}/${d.transport||'tcp'}`)+row('ORG',d.org||'N/A')+row('PRODUCT',d.product||'N/A')+row('OS',d.os||'N/A')+row('CITY',`${d.city||''} ${d.country||''}`)+vb+(d.snippet?`<div class="dp-banner">${d.snippet}</div>`:'')+`<a href="${d.link}" target="_blank" class="shodan-link">🔗 VIEW ON SHODAN</a>`;
  }
...
function row(k, v) { return `<div class="dp-row"><span class="dp-key">${k}</span><span class="dp-val">${v}</span></div>`; }
...
function getLODLevel(altM)   { return altM > LOD.GLOBAL ? 'GLOBAL' : altM > LOD.REGIONAL ? 'REGIONAL' : altM > LOD.CITY ? 'CITY' : 'STREET'; }

function updateCoords() {
  if (!S.viewer) return;
  const c   = S.viewer.camera.positionCartographic;
  const alt = c.height, km = alt / 1000;
...
function addLog(msg) {
  ...
  if (el) el.innerHTML = '<span class="log-label">EVT //</span>' + S.logHistory.slice(0, 3).map(m => `<span class="log-item">${m}</span>`).join('');
}
...
function setupUI() {
  ...
  satImgToggle?.addEventListener('change', () => {
    if (satImgToggle.checked) {
      ...
      else { if (satImgSelect) satImgSelect.value = 'modis_true'; setGibsLayer('modis_true'); }
    } else { setGibsLayer(null); }
  });
  satImgSelect?.addEventListener('change', e => {
    const key = e.target.value;
    if (key === 'none') { if (satImgToggle) satImgToggle.checked = false; setGibsLayer(null); }
    else { if (satImgToggle) satImgToggle.checked = true; setGibsLayer(key); }
  });
  ...
  // Vision mode buttons (kept in HTML, wired but now just post-processing shortcuts)
  document.querySelectorAll('.vision-btn').forEach(btn => btn.addEventListener('click', () => {
    ...
    document.getElementById('scanlines').style.opacity = { crt:'0.8', nvg:'0.5', flir:'0.4', radar:'0.3' }[mode] || '0';
    addLog(`DISPLAY: ${mode.toUpperCase()}`);
  }));
...
function setupFlightFilters() {
  ...
  const resetFilters = () => {
    ['f-min-alt','f-max-alt','f-min-spd','f-max-spd'].forEach((id, i) => {
      const el = document.getElementById(id);
      if (el) el.value = [0, 45000, 0, 1200][i];
    });
